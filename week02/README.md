...
- 미션 구현을 완료한 후 GitHub을 통해 제출해야 한다.
    ...
    - week02 패키지 내에 자신의 깃허브 이름(ex.KoSeonJe)로 패키지를 만들고 그 패키지 내부에
      (1) 과제 구현 코드(package로 된 하나의 파일)
      (2) 학습한 내용, 고민했던 점(md 파일)
      포함시켜서 제출해주시면 감사하겠습니다.

      ex)

        - week02
            - KoSeonJe.md
            - BankingSystem(한 개 패키지)
                - 구현코드~

    - pr 타이틀: [#주차]객체지향 코드 연습(깃허브 아이디)
        - ex. [2주차] 객체지향 코드 연습(KoSeonJe)
